<%-- 
    Document   : earchResult
    Created on : Mar 11, 2025, 3:28:41 PM
    Author     : Admin
--%>
...
<html>
    ...
    <body>
        ...
        <!-- comment -->  
        <h1>search result for ${searchinput}:(${gamesearchlistsize})</h1>
        
        <div class="container text-center w-90 h-90 border-top border-bottom  border-black border-4">
          <div class="row row-cols-3  gap-3 mt-3 ms-5 mb-3" >
            <!--  -->
            <c:forEach var = "c" items="${gamelistsearch}">
        ...
        <ul class="pagination">
            <c:if test = "${PNindexS > 1}">
               <li class="page-item active"><a class="page-link" href="search?pagenum=${PNindexS - 1}&searchinput=${searchinput}">Previous</a></li>
            </c:if>
            
          <c:forEach var = "i" begin="1" end="${totalpagesearch}">
            <li class="page-item active"><a class="page-link" href="search?pagenum=${i}&searchinput=${searchinput}">${i}</a></li>
          </c:forEach>
            <c:if test = "${PNindexS < totalpagesearch}">
                <li class="page-item active"><a class="page-link" href="search?pagenum=${PNindexS + 1}&searchinput=${searchinput}">Next</a></li>
            </c:if>
        </ul>
      </nav>
    </body>
